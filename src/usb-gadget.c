...
ep_matches(struct usb_gadget_dev_handle* handle,
    ...
    debug(handle, 2, "ep_matches: name:%s, d_addr:%02x, d_dir:%02x, d_type=%02x\n",
            name, desired_address, desired_direction, desired_type);
...
usb_gadget_endpoint(usb_gadget_dev_handle* handle, int number) {
    ...
    usb_gadget_list_for_each_entry (ep, &handle->ep_list, ep_list)if (
                (ep->descriptor->bEndpointAddress & USB_ENDPOINT_NUMBER_MASK) == number)
            return &ep->ep;
